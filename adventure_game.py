...
def print_pause(message, secs=2):
    print(message)
    time.sleep(secs)
...
def game_intro(antagonist):
    print_pause("You find yourself standing in "
                "an open field, filled with grass and yellow wildflowers.", 3)
    print_pause(f"Rumor has it that a {antagonist} is somewhere"
                " around here, and has been terrifying the nearby village.", 3)
...
def house(antagonist, weapon):
    print_pause("You approach the door of the house.", 2.5)
    print_pause("You are about to knock when"
                f" the door opens and out steps a {antagonist}.", 3)
    ...
    if "Sword of Ogoroth" in weapon:
        ...
        print_pause("You feel a bit under-prepared"
                    " for this, what with only having a tiny dagger.", 2.5)
...
# What happens when protagonist chooses to deliver
